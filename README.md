...
在 DeepSeek Harness 对话区边缘（左侧/右侧可调）添加迷你历史滑轨：悬停显示摘要，点击跳转历史请求位置。
...
- 垂直居中的迷你滑轨，每条记录 = 一条短横线（或圆点）
- 位置可切换：滑轨停靠对话区左侧或右侧（设置页「插件配置」卡片中修改），默认左侧
...
- **开启/关闭**：拨动「启用滑轨」开关
- **切换位置**：点击「左侧」或「右侧」按钮，滑轨停靠到对话区对应边缘
- **切换样式**：点击「横线」或「圆点」按钮
...
- **v1.1.10** — 中英双语：UI 文案接入内核 locale 服务——命名空间包 `shl-session-history` 注册进 `LocaleRuntime`（34 键 zh/en 全量对照，卡片/滑轨/更新流程全部文案），语言切换经 LocaleFace revision 实时重渲染；locale 服务缺席（旧版/精简部署）时按浏览器语言直查内置表兜底，查找链与内核同口径
- **v1.1.9** — 滚动联动高亮：滑轨实时常亮标记「当前视口在哪一轮」（焦点线取对话区顶部 25% 高度），节点经 `data-chat-anchor-key` 后缀启发式反查事件 id 映射到条目（长会话窗口化渲染下 DOM 序号不可用；映射不上宁可不亮）；高亮完全由滚动驱动，点击跳转后随平滑滚动从当前位置连续扫到目标、无瞬移停滞；标记用 `data-shl-act` 属性承载，不被悬停波浪的 className 重写冲掉，横线/圆点、左侧/右侧全模式生效；挂载时清扫同窗口孤儿滑轨/tooltip 节点（异常卸载兜底自愈），高亮同步链路异常隔离、不外抛内核 SlotErrorBoundary 波及会话视图
