window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// enabled：滑轨总开关；railStyle：横线 bar / 圆点 dot。
			const SETTINGS_KEY = "shl.settings";
			const SETTINGS_EVENT = "shl-settings-change";
			const localSettings = { enabled: true, railStyle: "bar" };
			...
			function ShlHistoryRail({ sessionId }) {
				...
				react.useEffect(() => {
					...
					settingsRef.current = { enabled: init.enabled, railStyle: init.railStyle };
				...
				const loadSettings = react.useCallback(() => {
					...
					const next = { enabled: s.enabled, railStyle: s.railStyle };
					const prev = settingsRef.current;
					if (next.enabled === prev.enabled && next.railStyle === prev.railStyle) return;
...
function ShlSettingsCard(props) {
	...
	const body = open ? react.createElement("div", {
		...
		row("启用滑轨", styleBtn(enabled, enabled ? "开" : "关", () => setEnabled(!enabled))),
			...
			ctx.effect(() => {
				const off = settingsScope.subscribe(() => {
					...
					if (prevSynced
						...
						&& prevSynced.railStyle === next.railStyle) return;
					prevSynced = next;
