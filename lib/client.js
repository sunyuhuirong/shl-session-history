window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// 此处读到的 snapshot 是 schema 解析后的 { enabled, railStyle }。
			const settingsScope = ctx.settingsScope.bind({ namespace: "shl-session-history" });
			...
			const localSettings = { enabled: true, railStyle: "bar", autoHide: true };
			...
			function ShlHistoryRail({ sessionId }) {
				...
				react.useEffect(() => {
					...
					settingsRef.current = { enabled: init.enabled, railStyle: init.railStyle, autoHide: init.autoHide };
				...
				const loadSettings = react.useCallback(() => {
					...
					const next = { enabled: s.enabled, railStyle: s.railStyle, autoHide: s.autoHide };
					const prev = settingsRef.current;
					if (next.enabled === prev.enabled && next.railStyle === prev.railStyle && next.autoHide === prev.autoHide) return;
			...
			ctx.effect(() => {
				const off = settingsScope.subscribe(() => {
					...
					if (prevSynced
						...
						&& prevSynced.railStyle === next.railStyle && prevSynced.autoHide === next.autoHide) return;
					prevSynced = next;
